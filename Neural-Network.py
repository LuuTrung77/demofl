...
# df = pd.read_csv('./UorLaptop.csv')

# # Tách dữ liệu thành đầu vào (X) và đầu ra (y)
# X = df[['RAM', 'SSD', 'CPU', 'Screen', 'Battery', 'Charging']]
# y = df['Price']
...
# joblib.dump(scaler, 'mpl_model.pkl')
import pandas as pd
import numpy as np
import pickle
from sklearn.model_selection import train_test_split
...
nn_clf = MLPRegressor().fit(X_train_full, y_train_full)
# Lưu mô hình Neural Network
with open('nn_model.pkl', 'wb') as file:
    pickle.dump(nn_clf, file)